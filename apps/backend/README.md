...
-   **Smart Import**: Processes adapter-driven CSV/XLSX imports with AI classification, deduplication, and staging for user review. The "Bijoy.ai Native CSV" system adapter supports direct import without AI — resolving account/category by name or ID from CSV columns.
...
**Setup for integration tests**: ensure `.env.test` has `DATABASE_URL` pointing to `bijoyai_test` and `INTERNAL_API_KEY` set, then run `npx prisma migrate deploy --schema prisma/schema.prisma` once against that database.
