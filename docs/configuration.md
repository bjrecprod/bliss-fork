...
|---|---|---|---|
| `DATABASE_URL` | Yes | `postgresql://bijoyai:changeme@localhost:5432/bijoyai` | PostgreSQL connection string used by Prisma in both services. Inside Docker the host is `postgres` instead of `localhost`. |
...
| `COOKIE_DOMAIN` | No | -- | Cookie domain for cross-subdomain auth (e.g., `.bijoy.ai`). Leave empty for localhost development. |
